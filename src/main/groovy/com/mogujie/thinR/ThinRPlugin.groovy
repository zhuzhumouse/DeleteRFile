        ...
        contextProvider.dexTask.doFirst {
            ...
            Collection<File> inputFile = contextProvider.getDexInputFile(new ContextProvider.Filter() {
                ...
                boolean accept(String path) {
                    //把Windows的路径转化为Linux的路径
                    path = path.replace("\\", "/")

                    boolean isMatch = false
                    if (path.startsWith(project.getRootDir().path.replace("\\", "/")) && path.contains("/build/")) {
                        isMatch = true
                    }

                    if (path.contains(".android/build-cache")) {
                        isMatch = true
                    }
...
//            PrintUtil.verbose("  " + "file--> " + file.absolutePath)
    ...
    void processJar(File jarFile, ThinRProcessor thinRProcessor) {
        ...
        PrintUtil.verbose("  " + "jarEntry dir:--> " + jarFile.absolutePath)
